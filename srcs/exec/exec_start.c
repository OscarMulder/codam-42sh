...
/*   Updated: 2019/06/25 19:02:18 by jbrinksm      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void		exec_start(t_ast *ast, t_envlst *envlst, int *exit_code)
{
	...
	if (ast->sibling != NULL)
	{
		if (ast->type == WORD || ast->type == ASSIGN)
			...
			ft_printf("skipped: %s\n", parser_return_token_str(ast->type));
	...
	if (ast->type == WORD)
	{
		...
		if (ast->type == WORD || ast->type == ASSIGN)
			...
			ft_printf("%s -- ", parser_return_token_str(ast->type));
		while (probe->child != NULL)
		{
			i++;
			probe = probe->child;
			if (probe->type == WORD || probe->type == ASSIGN)
				ft_printf("%s -- ", probe->value);
			else
				ft_printf("%s -- ", parser_return_token_str(probe->type));
		}
		ft_putendl("");
	}
	// else
