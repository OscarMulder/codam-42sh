...
/*   Updated: 2019/05/02 10:29:20 by rkuijper      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int		main(void)
{
	...
	if (test_var_get_value() != FUNCT_SUCCESS)
		return (test_ret_fail("test_var_get_value failed!"));
	...
	if (test_var_set_value() == FUNCT_FAILURE)
		return (test_ret_fail("test_var_set_value failed!"));
	if (test_var_join_key_value() == FUNCT_FAILURE)
		return (test_ret_fail("test_var_join_key_value failed!"));
	return (EXIT_SUCCESS);
}
